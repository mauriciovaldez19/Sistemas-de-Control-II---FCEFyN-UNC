...
% Tp N° 3 - Caso de estudio  - 
%   Inciso 3  
% Calcular sistema controlador que haga evolucionar al péndulo en el equilibrio estable.
% Objetivo de control: partiendo de una condición inicial nula en el desplazamiento y el ángulo en phi, hacer 
% que el carro se desplace a 10 metros evitando las oscilaciones de la masa m, considerando que es una 
% grúa. Una vez que delta=10 modificar a m a un valor 10 veces mayor y volver al origen evitando oscilaciones.
...
h=1e-4;tiempo=(15/h);p_pp=0;tita_pp=0;
...
Q = diag([1 1 1 1]);% Posicion Carro, Velocidad Carro, Angulo, Velocidad angular
R = 1;
...
x_hat=[0;0;pi;0]; %Inicializo el Observador
Xop=[0 0 pi 0]';
ref=10; 
psi(1)=0;
K=K_m1; G=G_m1;
...
%     else
        u(i)=-K*estado+Ki*psi(i+1);  %color='r';% LQR sin integrador ,sin observador
%         u(i)=-K*x_hat+G*ref; %color='b';% con observador
%     
%     end
%  
